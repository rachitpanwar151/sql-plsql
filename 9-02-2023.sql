...
SELECT UPPER(TO_CHAR(HIRE_DATE,'MONTH')) FROM EMPLOYEES EMP  ORDER BY TO_CHAR(EMP.HIRE_DATE,'MM') ;
...
Q-Write a quesry to fetch employee hire in leap year 
select last_name, HIRE_DATE FROM EMPLOYEES WHERE MOD(to_char(hire_date,'RRRR'),4)=0;
...
Q-print all employee  contains "A" without using substr and like
...
select emp.first_name||' '|| emp.last_name ||' hire on '|| (to_char(emp.hire_date, 'DD "of" MON RRRR'))|| ' with_salary ' ||to_char( emp.salary,'l99999d99')|| '  is working on department '||' '|| department_id from employees emp;
...
' AND HE IS WORKING ON DEPARTMENT '||RPAD(EMP.DEPARTMENT_ID,10,' ')) 
FROM EMPLOYEES EMP;
