create or replace PACKAGE BODY xxintg_validation_rp_FE AS
...
1.0           20-JULY-2023    RACHIT PANWAR           TO MAKE A PACKAGE BODY TO UPDATE AND
                                                      AND INSERT DETAILS IN EMPLOYEE
                                                      STAGING AND BASE TABLE
**************************************************************************************************/
...
------------------CHECKING CONDITION-----------------------------------
                ...
                FOR rec_report_validate IN cur_report_validate LOOP
                    BEGIN
                        fnd_file.put_line(fnd_file.output, rpad(nvl(to_char(rec_report_validate.employee_id), 'NUll'), 20, ' ')
                                                           ...
                                                           || '|'
                                                           || rpad(rec_report_validate.errorr, 300, ' '));
                ...
                FOR rec_error_report IN cur_report_error LOOP
                    BEGIN
                        fnd_file.put_line(fnd_file.output, rpad(nvl(to_char(rec_error_report.employee_id), 'Null'), 20, ' ')
                                                           || '|'
                                                           || rpad(rec_error_report.first_name, 50)
                                                           || '|'
                                                           || rpad(rec_error_report.last_name, 50)
                                                           || '|'
                                                           || rpad(rec_error_report.email, 50)
                                                           || '|'
                                                           || rpad(rec_error_report.phone_number, 50)
                                                           || '|'
                                                           || rpad(rec_error_report.hire_date, 50)
                                                           || '|'
                                                           || rpad(rec_error_report.job_id, 50)
                                                           || '|'
                                                           || rpad(rec_error_report.salary, 50)
                                                           || '|'
                                                           || rpad(rec_error_report.commission_pct, 50)
                                                           || '|'
                                                           || rpad(rec_error_report.manager_id, 50)
                                                           || '|'
                                                           || rpad(rec_error_report.department_id, 50)
                                                           || '|'
                                                           || rpad(rec_error_report.status, 50)
                                                           || '|'
                                                           || rpad(rec_error_report.errorr, 300));
        ...
        IF upper(p_process_type) = 'VALIDATE LOAD' THEN
            fnd_file.put_line(fnd_file.output, rpad('TOTAL RECORDS', 22)
                                               || ' :- '
                                               || ln_tcount);

            fnd_file.put_line(fnd_file.output, rpad('TOTAL SUCCESS RECORDS', 22)
                                               || ' :- '
                                               || ln_scount);
            ...
            fnd_file.put_line(fnd_file.output, ' ');
            IF ln_scount = 0 THEN
                fnd_file.put_line(fnd_file.output, ' NO success record FOUND');
            ELSE
                fnd_file.put_line(fnd_file.output, '**********SUCCESS REPORT************');
                fnd_file.put_line(fnd_file.output, rpad('EMPLOYEE_ID', 20, ' ')
                                                   || '|'
                                                   || rpad('FIRST_NAME', 20, ' ')
                                                   || '|'
                                                   || rpad('LAST_NAME', 20, ' ')
                ...
                FOR rec_report_success IN cur_report_success LOOP
                    BEGIN
                        fnd_file.put_line(fnd_file.output, rpad(rec_report_success.employee_id, 20, ' ')
                                                           || '|'
                                                           ...
                                                           || '|'
                                                           || rpad(rec_report_success.errorr, 300, ' '));

                    EXCEPTION
                        WHEN OTHERS THEN
                            fnd_file.put_line(fnd_file.output, 'ERROR IN PRINTING SUCCESS REPORT');
                    END;
                END LOOP;
                ...
                FOR rec_error_report IN cur_report_error LOOP
                    BEGIN
                        fnd_file.put_line(fnd_file.output, rpad(nvl(to_char(rec_error_report.employee_id), 'Null'), 20, ' ')
                                                           ...
                                                           || '|'
                                                           || rpad(rec_error_report.errorr, 500));

                    EXCEPTION
                        WHEN OTHERS THEN
                            fnd_file.put_line(fnd_file.output, ' ERROR IN PRINTING ERROR REPORT'
                                                               || '|'
                                                               || sqlcode
                                                               || '|'
                                                               || sqlerrm);
...
----->DECLARING CURSOR FOR VALIDATION
    ...
    BEGIN
        fnd_file.put_line(fnd_file.output, 'batch id in validation' || p_batch_id);
...
******************************/
                ...
                ELSIF length(rec_emp_stag_dtl.first_name) > 50 THEN
                    lc_status := 'E';
                    lc_error := lc_error
                                || ' '
                                || '-FIRST_NAME CAN ONLY HAVE 20 CHARACTER';
                END IF;
...
******************************/
                ...
                ELSIF length(rec_emp_stag_dtl.last_name) > 50 THEN
                    lc_status := 'E';
                    lc_error := lc_error
                                || '|'
...
**********************************************/
                ...
                ELSIF rec_emp_stag_dtl.salary < 500 THEN
                    lc_status := 'E';
                    lc_error := lc_error
                                || '|'
                                || '-MINIMUM  SALARY SHOULD BE  500';
                END IF;
...
************************************************/
    ...
    BEGIN
        ...
        , argument2 => '/tmp/RPANWAR/EMP_RP/', 
        argument3 => p_file_name||'.csv',
        argument4 => '/tmp/RPANWAR/EMP_RP/emp_data_rp.log'
        , argument5 => '/tmp/RPANWAR/EMP_RP/emp_data_rp.bad',
        argument6 => '/tmp/RPANWAR/EMP_RP/emp_data_rp.dis', 
        argument7 => '/tmp/RPANWAR/EMP_RP');
            ...
            IF (
                ...
                BEGIN
                    ...
                    SET
                        ...
                        /*
                        create sequence recore_seq_rp
                        start with 1
                        increment by 1;*/
                        record_id = recore_seq_rp.NEXTVAL
                    WHERE
                        status = 'N';
...
***************************************/
            ...
            EXCEPTION
                WHEN OTHERS THEN
                    ...
                    SET
                        ...
                        errorr = lc_e
                    WHERE
                        record_id = rec_emp_dtl.record_id;
    ...
    EXCEPTION
        WHEN OTHERS THEN
            fnd_file.put_line(fnd_file.output, ' ERROR OCCURED IN BASE TABLE INSERT PROCESS'
                                               || '|'
                                               || sqlcode
                                               || '|'
                                               || sqlerrm);
...
truncate table APPS.XXINTG_STAGING_RP;