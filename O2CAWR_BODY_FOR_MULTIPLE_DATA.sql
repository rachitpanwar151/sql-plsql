...
--                AND st2.request_id = gn_request_id;
                AND ST2.LINE_id=p_line_id
;

DBMS_OUTPUT.PUT_LINE('CANCELLED LINE ID IS '||p_line_id);
            COMMIT;
        EXCEPTION
            WHEN OTHERS THEN
...
;
            COMMIT;
            DBMS_OUTPUT.PUT_LINE('CANCELLED HEADER ID IS '||p_header_id);
        EXCEPTION
            WHEN OTHERS THEN
...
AND UPPER(FLOW_STATUS_CODE_LINE)='AWAITING_SHIPPING'
AND UPPER(STATUS)='CANCEL LINE'
...
v_line_tbl (1).cancelled_flag       := upper(r_cancel(I).cancel_flag);
...
ELSE
    ...
    END LOOP;
END IF;
           update_order_cancel_LINES_details(r_cancel(i).order_header_id,R_CANCEL(I).LINE_ID);
...
--            dbms_output.put_line('cursor ka loop kay andar aya');
            ...
            FOR i IN 1..so_data_hdr.count LOOP
                ...
                /*              
                CREATE SEQUENCE RECORD_ID_AWR_HDR
                START WITH 1
                INCREMENT BY 1;
                SELECT RECORD_ID_AWR_HDR.NEXTVAL FROM DUAL;
                                */
                so_data_hdr(i).RECORD_ID:=RECORD_ID_AWR_HDR.NEXTVAL;
...
--
--
--                        IF so_data_hdr(i).SHIP_TO_LOCATION_NAME IS NULL THEN
--                            so_data_hdr(i).status_msg := 'E';
--                            so_data_hdr(i).error_msg := so_data_hdr(i).error_msg
--                                                         || 'SHIP TO LOCATION NAME CAN NOT BE NULL'
--                                                         || sqlcode
--                                                         || sqlerrm;
--                                                         
--                ELSE
--                BEGIN
--                
--                SELECT SITE_USE_ID INTO so_data_hdr(i).SHIP_TO_LOCATION_ID
--                FROM HZ_CUST_SITE_USES_ALL 
--                WHERE UPPER(LOCATION)=UPPER(so_data_hdr(i).SHIP_TO_LOCATION_NAME)
...
--                            so_data_hdr(i).status_msg := 'E';
--                            so_data_hdr(i).error_msg := so_data_hdr(i).error_msg
--                                                        || 'ERROR IN VALIDATING SHIP TO LOCATION ID'
--                                                        || sqlcode
--                                                        || sqlerrm;
--
--                    END;
...
--
--
--                        IF so_data_hdr(i).BILL_TO_LOCATION_NAME IS NULL THEN
--                            so_data_hdr(i).status_msg := 'E';
--                            so_data_hdr(i).error_msg := so_data_hdr(i).error_msg
--                                                         || 'BILL TO LOCATION NAME CAN NOT BE NULL'
--                                                         || sqlcode
--                                                         || sqlerrm;
--                                                         
--                ELSE
--                BEGIN
--                
--                SELECT SITE_USE_ID INTO so_data_hdr(i).BILL_TO_LOCATION_ID
--                FROM HZ_CUST_SITE_USES_ALL 
--                WHERE UPPER(LOCATION)=UPPER(so_data_hdr(i).BILL_TO_LOCATION_NAME)
...
--                            so_data_hdr(i).status_msg := 'E';
--                            so_data_hdr(i).error_msg := so_data_hdr(i).error_msg
--                                                        || 'ERROR IN VALIDATING BILL TO LOCATION ID'
--                                                        || sqlcode
--                                                        || sqlerrm;
--
--                    END;
...
--                        dbms_output.put_line('fetch k line loop k andar ayaa');
                ...
                /*              
                CREATE SEQUENCE RECORD_ID_LINE_AWR
                START WITH 1
                INCREMENT BY 1;
                SELECT RECORD_ID_LINE_AWR.NEXTVAL FROM DUAL;
                                */
                        so_data_line(i).record_id:=RECORD_ID_LINE_AWR.NEXTVAL;
	...
	******************************/
                        ...
                        BEGIN
                            ...
                            SET
                                ...
                                request_id = gn_request_id,
                       RECORD_ID=so_data_line(i).record_id 
                       WHERE
                                SF_ORDER_NUMBER = so_data_line(i).SF_ORDER_NUMBER
								and INVENTORY_item_name=so_data_line(i).INVENTORY_item_name;
                ...
                CLOSE C_LINES;
--       dbms_output.put_line('exit hua  line k validation say');
                ...
                BEGIN
                    ...
                    SET
                        ...
                        request_id = gn_request_id,
                        RECORD_ID=so_data_hdr(i).RECORD_ID
                    WHERE
...
--        dbms_output.put_line('exit hua header and line k validation say');
 ...
 PROCEDURE main_procedure (
    ...
    ln_count1 NUMBER;    
--    LC_STATUS_MSG VARCHAR2(250);
--    LC_ERROR_MSG VARCHAR2(250);
    
   CURSOR C_HDR
   IS
   SELECT DISTINCT * FROM XXAWR_STAGING_HEADER_SO WHERE 
   OIC_INTERFACE_ID=p_OIC_interface_id;
    
   CURSOR C_LINE
   IS
   SELECT DISTINCT * FROM XXAWR_STAGING_LINE_SO WHERE 
   OIC_INTERFACE_ID=p_OIC_interface_id
   AND UPPER(CANCEL_FLAG)='Y'
   AND STATUS='CANCEL LINE';
...
****************************************/
  SELECT
    ...
    WHERE
            oic_interface_id = p_oic_interface_id
        AND record_id = r_hdr.record_id
        AND sf_order_number = r_hdr.sf_order_number;
 
/****************************************
  Condiction For Checking The Status
******************************************/
IF upper(lc_STATUS)='DRAFT'
THEN
...
ELSE IF upper(lc_STATUS)='CANCEL'
THEN
       sales_order_creation(P_OIC_INTERFACE_ID);
       SALES_ORDER_BOOKING(P_OIC_INTERFACE_ID);
BEGIN
    ...
    WHERE
            oic_interface_id = p_oic_interface_id
        AND upper(flow_status_code_hdr) = 'BOOKED'
        AND UPPER(CANCEL_FLAG)='Y'
        AND UPPER(STATUS)='CANCEL';
           ...
           SELECT
                  COUNT(DISTINCT order_header_id) 
                    INTO ln_count
                          ...
                          xxawr_staging_line_so
                            WHERE
                              upper(status) = 'CANCEL LINE'
                     AND upper(cancel_flag) = 'Y'
            AND upper(flow_status_code_line) = 'AWAITING_SHIPPING'
          AND LINE_ID=R_LINE.LINE_ID;
...
SELECT STATUS_MSG,ERROR_MSG INTO P_STATUS , P_ERROR
FROM XXAWR_STAGING_HEADER_SO WHERE OIC_INTERFACE_ID=p_oic_interface_id;
    EXCEPTION
            WHEN OTHERS THEN
                dbms_output.put_line('Unexpected error in HEADER STATUS AND ERRROR MESSAGE OUT'
                                     || '-'
                                     || sqlcode
                                     || '-'
                                     || sqlerrm);
...
FROM XXAWR_STAGING_HEADER_SO WHERE OIC_INTERFACE_ID=p_oic_interface_id;
    ...
    EXCEPTION
            WHEN OTHERS THEN
                dbms_output.put_line('Unexpected error in OUTING STATUS AND ERROR MSG IN LINE'
                                     || '-'
                                     || sqlcode
                                     || '-'
                                     || sqlerrm);
