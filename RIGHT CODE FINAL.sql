...
CREATE OR REPLACE PACKAGE BODY apps.XXAWR_SO_SF_OIC_PKG
AS
   /*******************************************************************************************************************
    * $Header         : @(#) 
...
***********************************************************************/
   ...
   * WHO              WHAT                                                          WHEN
   * --------------   -------------------------------------------------------       --------------
   *                    Procedure TO INSERT DATA FROM INTO TO STAGING                 2024
   *******************************************************************************************************************/
      ...
      THEN
         ...
         BEGIN
           ...
           EXCEPTION
            WHEN OTHERS
            THEN
            DBMS_OUTPUT.PUT_LINE ('Error While Insserting Header staging data from Interface table:'
                  || SQLCODE
                  || '-'
                  || SQLERRM);
         END;
		 ...
		 EXCEPTION
            ...
            THEN
               DBMS_OUTPUT.PUT_LINE (
                  'Error While Insserting Line staging data from Interface table:'
                  || SQLCODE
                  || '-'
                  || SQLERRM);
         END;
         ...
         BEGIN
            ...
            WHEN OTHERS
            THEN
             DBMS_OUTPUT.PUT_LINE ('Error While Insserting Delivery staging data from Interface table:'
                  || SQLCODE
                  || '-'
                  || SQLERRM);
         END;
   ...
   EXCEPTION
      WHEN OTHERS
      THEN
         DBMS_OUTPUT.PUT_LINE ('Main Error at STAGING_INSERT_PRC: ' || SQLCODE || '-' || SQLERRM);
    ...
    * WHO              WHAT                                                            WHEN
    * --------------   -------------------------------------------------------       --------------
    *                    Procedure TO VALIDATE APPS ID , RESP APP ID AND USER ID        2024
    *******************************************************************************************************************/
                ...
                (SELECT stg_hd1.sf_order_id
                 FROM xxaac.xxawr_so_lines_stg stg_ln1,
                      ...
                      xxaac.xxawr_so_delvy_instr_stg stg_del1
                         ...
                         AND stg_del1.STG_HEADER_ID = stg_hd1.STG_HEADER_ID
                         AND stg_ln1.STG_LINE_ID = stg_del1.STG_LINE_ID
                         AND UPPER (stg_del1.stg_del_instr_status) ='CANCELLED'
                         AND stg_del1.status = 'N'
                         AND stg_del1.oic_instance_id = IN_P_OIC_INSTANCE_ID
                         AND stg_del1.stg_del_instr_id = stg_del.stg_del_instr_id);

      TYPE c_so_cancel_type IS TABLE OF c_so_cancel%ROWTYPE INDEX BY PLS_INTEGER;
   ...
   BEGIN
      ...
      AND stg_del.stg_del_instr_id =
                    ...
                    AND stg_del1.STG_HEADER_ID = stg_hd1.STG_HEADER_ID
                    AND stg_ln1.STG_LINE_ID = stg_del1.STG_LINE_ID
                    AND LOWER (stg_del1.stg_del_instr_status) = 'draft'
                    AND EXISTS
                        (SELECT 1
                         FROM xxaac.xxawr_so_lines_stg stg_ln
                         WHERE stg_del.STG_HEADER_ID = stg_ln.STG_HEADER_ID
                         AND stg_ln.STG_LINE_ID = stg_del.STG_LINE_ID
                         AND UPPER (stg_ln.item_code) = UPPER (stg_ln1.item_code)
                         AND stg_ln.OIC_INSTANCE_ID = in_p_oic_instance_id)
                         AND stg_hd1.sf_order_id = 
						                 (SELECT sf_order_id
                                          FROM xxaac.xxawr_so_header_stg stg_hd
                                          WHERE stg_del.stg_header_id = stg_hd.stg_header_id
                                          AND stg_hd.OIC_INSTANCE_ID = IN_P_OIC_INSTANCE_ID));
      ...
      BEGIN
         ...
         AND EXISTS
             (SELECT 1
              ...
              AND stg_ln.STG_LINE_ID = stg_del.STG_LINE_ID
              AND UPPER (stg_ln.item_code) =  UPPER (stg_ln1.item_code)
              AND stg_ln.OIC_INSTANCE_ID = stg_del.oic_instance_id)
              AND stg_del.stg_del_instr_id = stg_del1.stg_del_instr_id
              AND LOWER (stg_del1.stg_del_instr_status) = 'cancelled'
              AND stg_del1.oic_instance_id = in_p_oic_instance_id
              AND stg_hd1.sf_order_id =
                               (SELECT sf_order_id
                                FROM xxaac.xxawr_so_header_stg stg_hd
                                WHERE stg_del.stg_header_id = stg_hd.stg_header_id
                                AND stg_hd.OIC_INSTANCE_ID = stg_del.oic_instance_id))
                                AND EXISTS
                                (SELECT 1
   ...
   *******************************************************************************************************************/
   ...
   BEGIN
      ...
      UPDATE xxaac.xxawr_so_header_stg
         SET oracle_ref_id = 'ORACLE_ID-' || STG_HEADER_ID
       WHERE     oic_instance_id = in_p_oic_instance_id
             AND status = 'N'
   ...
   ********************************************/
    ...
    /********************************************
      ...
      UPDATE xxaac.xxawr_so_lines_stg
         SET ORACLE_LINE_REF_ID = 'ORACLE_LINE_REF_ID-' || STG_LINE_ID
       WHERE     oic_instance_id = in_p_oic_instance_id
             AND status = 'N'
      ...
      ******************************/
       ...
       ********************************************/
      UPDATE xxaac.xxawr_so_DELVY_INSTR_stg
         SET ORACLE_DEL_REF_ID = 'ORACLE_LINE_REF_ID-' || RECORD_ID
       WHERE     oic_instance_id = in_p_oic_instance_id
             AND status = 'N'
   ...
   *******************************************************************************************************************/
   ...
   BEGIN
      ...
      LOOP
         ...
         DBMS_OUTPUT.put_line (
            'REC_HEADER.TRANSACTIONAL_CURR_CODE is   '
            || REC_HEADER.TRANSACTIONAL_CURR_CODE);
         DBMS_OUTPUT.put_line (
            'REC_HEADER.CUSTOMER_ID is   ' || REC_HEADER.CUSTOMER_ID);
         DBMS_OUTPUT.put_line (
            'REC_HEADER.PRICE_LIST_ID is   ' || REC_HEADER.PRICE_LIST_ID);
         DBMS_OUTPUT.put_line (
            'REC_HEADER.SALESREP_ID is   ' || REC_HEADER.SALESREP_ID);
         DBMS_OUTPUT.PUT_LINE (
            'REC_HEADER.ORDERED_TYPE_ID      ' || REC_HEADER.ORDERED_TYPE_ID);

         DBMS_OUTPUT.PUT_LINE (
            'REC_HEADER.SF_ORDER_ID    ' || REC_HEADER.SF_ORDER_ID);
         DBMS_OUTPUT.PUT_LINE (
            ' REC_HEADER.STG_HEADER_ID   ' || REC_HEADER.STG_HEADER_ID);
         ...
         LOOP
            ...
            DBMS_OUTPUT.put_line (REC_LINE.schedule_ship_date);



            DBMS_OUTPUT.PUT_LINE (
               'ORDERED_QUANTITY' || REC_LINE.quantity_to_be_delivered);
            DBMS_OUTPUT.PUT_LINE (
               'REC_LINE.SHIP_FROM_ORG_ID' || REC_LINE.SHIP_FROM_ORG_ID);
            DBMS_OUTPUT.PUT_LINE ('REC_LINE.ITEM_ID' || REC_LINE.ITEM_ID);
            DBMS_OUTPUT.PUT_LINE (
               'REC_LINE.schedule_ship_date' || REC_LINE.schedule_ship_date);
            ...
            L_LINE_TBL (L_LINE_TBL_INDEX).CALCULATE_PRICE_FLAG := 'N';
              L_LINE_TBL (L_LINE_TBL_INDEX).TAX_VALUE := REC_LINE.VAT_AMOUNT; --Adding VAT Amount
	...
	--->  P_LINE_ADJ_TBL.DISCOUNT :=   REC_LINE.DISCOUNT                    ----adding discount amount 
         ...
         THEN
            ...
            THEN
               ...
               UPDATE xxaac.xxawr_so_delvy_instr_stg
                  SET STATUS = 'E',
                      ERROR_MSG = 'Error in booking Sales Order - ' || L_err_msg
                WHERE 1 = 1
                      AND OIC_INSTANCE_ID = IN_P_OIC_INSTANCE_ID
                      AND STG_HEADER_ID = REC_HEADER.STG_HEADER_ID
                      AND UPPER (STG_DEL_INSTR_STATUS) = 'BOOKED';
            ELSE
               ...
               UPDATE xxaac.xxawr_so_header_stg
                  SET STATUS = 'S',
                      ORACLE_HEADER_STATUS = (SELECT flow_STATUS_code
               ...
               LOOP
                  UPDATE xxaac.xxawr_so_delvy_instr_stg stg_del
                     SET STATUS = 'S',
                         ORACLE_ORDER_HEADER_ID = X_HEADER_REC.HEADER_ID,
                         ORACLE_ORDER_NUMBER = X_HEADER_REC.ORDER_NUMBER,
                         oracle_order_line_id = (SELECT line_id
                               FROM oe_order_lines_all oe_l
                              WHERE oe_l.header_id = X_HEADER_REC.HEADER_ID
                                    AND oe_l.attribute2 = rec_line_c.sf_line_num)
                   WHERE     1 = 1
                         AND OIC_INSTANCE_ID = IN_P_OIC_INSTANCE_ID
                         AND STG_HEADER_ID = REC_HEADER.STG_HEADER_ID
                         AND UPPER (STG_DEL_INSTR_STATUS) = 'BOOKED'
         ...
         ELSE
            ...
            BEGIN
               ...
               ********************************/
               UPDATE xxaac.xxawr_so_header_stg
                  SET STATUS = 'E',
                      ERROR_MSG = 'Error in creating Sales Order'
                         || '    '
                         || L_API_ERROR_MESSAGE
                WHERE     1 = 1
                      AND OIC_INSTANCE_ID = IN_P_OIC_INSTANCE_ID
               ...
               UPDATE xxaac.xxawr_so_lines_stg stg_line
                  SET STATUS = 'E',
                      ERROR_MSG = 'Error in creating Sales Order - ' || L_err_msg
                WHERE     1 = 1
                      AND stg_line.OIC_INSTANCE_ID = IN_P_OIC_INSTANCE_ID
                      AND STG_HEADER_ID = REC_HEADER.STG_HEADER_ID
                      AND EXISTS
   ...
   AS
      ...
      IS
         SELECT XSLS.*,
                ...
                XSDIS.ATTRIBUTE1 org_id
             --   XSDIS.reservation_quantity
           FROM xxaac.xxawr_so_lines_stg XSLS,
                xxaac.xxawr_so_delvy_instr_stg XSDIS
          WHERE     1 = 1
                AND XSLS.OIC_INSTANCE_ID = IN_P_OIC_INSTANCE_ID
      ...
      LOOP
         ...
         LOOP
            BEGIN
               ...
               p_rsv.organization_id := CRT_RES_TBL (INDX).org_id; --mtl_parameters.organization id
      ...
      LOOP
         ...
         LOOP
            BEGIN
               ...
               -----------------------------------------------------
               IF (INV_RES_TBL (indx).ORGANIZATION_NAME IS NOT NULL)
               THEN
                  BEGIN
                     SELECT organization_id
                      ...
                      WHERE LOWER (organization_name) =
                               LOWER (INV_RES_TBL (indx).organization_name);

                     DBMS_OUTPUT.put_line ('ITEM RESV VALIDATION ORG NAME');
                  EXCEPTION
                     WHEN OTHERS
                     THEN
                        INV_RES_TBL (INDX).ERROR_MSG :=
               ...
               *********************************************/
               ...
               THEN
                  BEGIN
                     ...
                     DBMS_OUTPUT.put_line ('ITEM RESV VALIDATION ITEM CODE');
                  EXCEPTION
                     WHEN OTHERS
                     THEN
                        INV_RES_TBL (INDX).ERROR_MSG :=
               ...
               ***********************************************/
                  ...
                  UPDATE xxaac.xxawr_so_delvy_instr_stg
                     SET                                --who columns updation
                        CREATION_DATE = SYSDATE,
                         ...
                         ,
                         attribute1 = INV_RES_TBL (INDX).ORGANIZATION_ID
                   WHERE     1 = 1
                         AND STATUS = 'N'
               ...
               EXCEPTION
                  ...
                  THEN
                     DBMS_OUTPUT.PUT_LINE (
                           'Unexcpected Erros while Updating Staging table '
                        || SQLCODE
                        || ' - '
                        || SQLERRM);
               END;
   ...
   BEGIN
       ...
       WHERE     UPPER (STG_DEL_INSTR_STATUS) = 'CANCELLED'
             ...
             AND EXISTS
                    (SELECT 1
                      ...
                      WHERE 1 = 1 --oha.header_id = stg_hd.oracle_order_header_id
                                 );
      ...
      ELSE
         ...
         LOOP
            ...
            DBMS_OUTPUT.put_line (r_stg_del_instr_status.stg_del_instr_id);
            ...
            THEN
               ...
               ITEM_RES_INV_VALIDATION_PRC (
                  ...
                  DBMS_OUTPUT.put_line (
                     'Calling ITEM_RES_INV_VALIDATION_PRC process.');
                  DBMS_OUTPUT.put_line (
                     'Calling CREATE_RESERVATION_API_PRC process.');
               ELSIF UPPER (r_stg_del_instr_status.stg_del_instr_status) =
                        'UNRESERVE'
               THEN
                  DBMS_OUTPUT.put_line ('UNRESERVATION PROCESS CALLING -');
                  ITEM_RES_INV_VALIDATION_PRC (
                     IN_P_OIC_INSTANCE_ID,
                     R_STG_DEL_INSTR_STATUS.STG_DEL_INSTR_STATUS);
               ...
               THEN
                  ...
                  LOOP
                     ...
                     BEGIN
                        ...
                        DBMS_OUTPUT.put_line ('Staging Validation');
                        XXAWR_STAGING_VALIDATION (IN_P_OIC_INSTANCE_ID);
                        DBMS_OUTPUT.put_line (
                           'Item Unreservation Check before Order creation');
                        UNRESERVE_ITEM_API_PRC (IN_P_OIC_INSTANCE_ID); --HRAWAT 18 Apr: Unreserve Item before Sales order creation
                        DBMS_OUTPUT.put_line ('Order Creation');
                        SALES_ORDER_CREATION (IN_P_OIC_INSTANCE_ID);
                        DBMS_OUTPUT.put_line ('Order Booking');
                     END;
                  END LOOP;
   ...
   *******************************************************************************************************************/
   ...
   BEGIN
      ...
      staging_insert_prc (in_p_oic_instance_id); --Load Stagging table process
